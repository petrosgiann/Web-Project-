...
function displayProductsByCategory(products) {
    ...
    products.forEach(product => {
        ...
        quantityInput.min = "0";
        quantityInput.max =product.quantity_available;
        ...
        updateButton.addEventListener("click", function () {
            ...
            if (!isNaN(addQuantity) && addQuantity >= 0) {
                makeNewRequest(product.product_id,  addQuantity);
            }
        });
...
function makeNewRequest(productId, addedQuantity) {
    fetch(`newrequest.php?product=${productId}&quantity=${addedQuantity}`)
    .then(response => response.json())
    .then(data => {
